...
- **No `.mcp.json` is checked in.** This build is not published to npm, so there is no registry ref to point one at. Build the CLI, `npm link` it, then run `gitnexus setup` inside the container — it resolves the linked binary and writes the MCP entry itself.
