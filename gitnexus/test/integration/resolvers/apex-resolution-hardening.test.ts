...
describe.skipIf(!apexAvailable)('Apex resolution-path hardening — adversarial corpus (Gate 5, §A.4)', () => {
...
describe.skipIf(!apexAvailable)('Apex resolution-path hardening — bounded smoke-fuzz (Gate 5, §A.4)', () => {
  ...
  // just inert bytes.
  const ATOMS = [
    'class C {', 'void m() {', '}', 'foo(', 'a.b(', 'a.b.c', 'new Account(',
    'this(', 'super(', 'x', ', y', 'Account a =', 'Integer i = 1;', "String s = 'x';",
    'extends B', 'implements I', '(', ')', '.', ';', ',', '<Account>', 'on Account',
    'trigger T', 'interface I {', 'enum E {', ' ', '\n',
  ];
