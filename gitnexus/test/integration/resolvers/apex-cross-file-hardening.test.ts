...
describe.skipIf(!apexAvailable)('Apex cross-file hardening — adversarial corpus (Gate 5, §A.4)', () => {
...
describe.skipIf(!apexAvailable)('Apex cross-file hardening — bounded smoke-fuzz (Gate 5, §A.4)', () => {
  ...
  const ATOMS = [
    'class C {', 'enum E {', 'void m() {', '}', 'Outer.Inner', 'new Outer.Inner(',
    'Outer.', '.Inner', 'i;', 'f(', 'this.w', '7', 'true', 'super.', 'inherited(',
    'extends Outer.Inner', 'A, B,', 'RED', ', ', '(', ')', '.', ';', ' ', '\n',
  ];
