...
interface ConformanceCase {
   ...
   * why those resolvers cannot tell the two apart. Omitted too by a language
   * with {@link noImportMechanism}, which can reach no file by any spelling.
   */
  readonly reachesDecoy?: string;
  /**
   * The language has no import statement at all, so its `resolveImportTarget`
   * answers `null` for every specifier and the property below holds by
   * construction rather than by an algorithm.
   *
   * Such a language is exempt from the decoy-reachability arm ONLY — the arm
   * asks whether a tempting wrong answer was reachable, and for a resolver that
   * reaches nothing the question has no content; requiring a `reachesDecoy`
   * would mean inventing a spelling the language does not have. The external
   * arm still runs unchanged, which is what keeps the exemption honest: a
   * resolver that starts returning a file fails there.
   */
  readonly noImportMechanism?: true;
...
const CASES: ReadonlyMap<SupportedLanguages, ConformanceCase> = new Map([
  ...
  [
    ...
    {
      // Apex has no import statement — every top-level type is visible
      // org-wide — so `apexScopeResolver.resolveImportTarget` is `() => null`
      // and never consults the file set. The decoy is still a real Apex path
      // ending the way the specifier does, so the external arm measures the
      // resolver against the same temptation as every other language.
      files: ['classes/Database.cls', 'classes/AccountService.cls', 'classes/App.cls'],
      fromFile: 'classes/App.cls',
      resolutionConfig: undefined,
      external: 'System.Database',
      decoy: 'classes/Database.cls',
      noImportMechanism: true,
    },
  ],
]);
