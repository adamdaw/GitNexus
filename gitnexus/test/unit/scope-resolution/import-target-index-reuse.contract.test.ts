...
/**
 * Languages with no import statement to resolve.
 *
 * A third kind of entry, and the weakest claim of the three: `KNOWN_UNINDEXED`
 * says a resolver derives nothing from the file set, while this says the
 * language never asks it anything. Apex makes every top-level type visible
 * org-wide, so there is no specifier to resolve and `resolveImportTarget` is a
 * constant `null`.
 *
 * The exemption is verified rather than granted — the arm below drives a real
 * specifier through each listed resolver and fails if one answers with a file.
 * A language cannot sit here and quietly grow an import algorithm.
 */
const NO_IMPORT_MECHANISM: ReadonlyMap<SupportedLanguages, string> = new Map<
  SupportedLanguages,
  string
>([[SupportedLanguages.Apex, 'no import statement — every top-level type is visible org-wide']]);
...
describe('import-target index reuse — the contract every registered resolver holds', () => {
  ...
  it('covers every registered scope resolver', () => {
    ...
    const accountedFor = [
      ...FIXTURES.keys(),
      ...KNOWN_UNINDEXED.keys(),
      ...NO_IMPORT_MECHANISM.keys(),
    ].sort();

    // A new language in `pipeline/registry.ts` lands here first: it is either
    // given a fixture in `FIXTURES` or an entry in `KNOWN_UNINDEXED` /
    // `NO_IMPORT_MECHANISM`, and all three are edits someone has to justify.
    expect(accountedFor).toEqual(registered);
  });

  it.each([...NO_IMPORT_MECHANISM.keys()])(
    '%s: has no import statement, so its resolver answers nothing',
    (language) => {
      // The exemption above, measured. A resolver that grew an import algorithm
      // would resolve something here and have to move to `FIXTURES`.
      const resolver = SCOPE_RESOLVERS.get(language)!;
      const resolved = resolver.resolveImportTarget(
        'System.Database',
        'classes/App.cls',
        new Set(['classes/App.cls', 'classes/Database.cls']),
        undefined,
        { parsedFiles: [], parsedImport: undefined },
      );
      expect(
        resolved,
        `${language}: NO_IMPORT_MECHANISM records no import resolution, but a specifier resolved to ${JSON.stringify(resolved)}`,
      ).toBeNull();
    },
  );
