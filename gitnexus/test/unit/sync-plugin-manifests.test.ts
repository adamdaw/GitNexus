...
describe('syncPluginManifests (#2445)', () => {
  ...
  // The plugin skill mcp.json are executable: they launch a server when a skill
  // starts. This build is not published to npm, so a `gitnexus@<version>` launch
  // arg would resolve to the published package — no Apex support, and it returns
  // nothing rather than erroring. They must launch the PATH-resolved binary, and
  // so carry no version for a release to stamp (hence not a sync surface above).
  it('every shipped plugin skill mcp.json launches the PATH binary, never npm', () => {
    for (const dir of MCP_SKILL_DIRS) {
      const mcp = JSON.parse(readFileSync(path.join(REPO_ROOT, mcpPath(dir)), 'utf8')) as {
        mcpServers: { gitnexus: unknown };
      };
      expect(mcp.mcpServers.gitnexus).toEqual({ command: 'gitnexus', args: ['mcp'] });
    }
  });
