...
export function runScopeResolution(
  ...
  // the first would make dispatch depend on file order. Declared outside
  // `runHeritageAndMro` because the downstream tail reads it.
  ...
  const runHeritageAndMro = (scopesForHeritage: typeof finalized) => {
    ...
    const postHeritageNodeLookup =
      !callableFlowOnly && provider.emitHeritageEdges !== undefined
        ...
        : nodeLookup;
    if (!callableFlowOnly) {
      // The interfaces the provider could not decide (#2873) — no edges, a
      // reportable lower bound.
