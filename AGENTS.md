...
> Index stale? Run `node .gitnexus/run.cjs analyze` from the project root — it runs the `gitnexus` binary on PATH. No `.gitnexus/run.cjs` yet? Build from source and re-run — the npm-published build has no Apex support and reports no error.
